...
- One `StatefulSet` (replicas: 1) per entry in `values.repos`, running the
  ...
  - a 1Gi `emptyDir` mounted at `/tmp` on the agent container (`storage.tmpSize`) —
...
| storage | object | `{"homeSize":"5Gi","storageClassName":"","tmpSize":"1Gi","workspaceSize":"5Gi"}` | Default persistent volume sizes for each per-repo agent. |
...
| storage.tmpSize | string | `"1Gi"` | sizeLimit of the emptyDir mounted at /tmp on the agent container (e.g. for cc-rc-pr-update's PR summary/plan/ledger drafts). |

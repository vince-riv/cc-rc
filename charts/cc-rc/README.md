...
- One `StatefulSet` (replicas: 1) per entry in `values.repos`, running the
  ...
  - a 2Gi `emptyDir` mounted at `/tmp` on the agent container (`storage.tmpSize`) —
    e.g. for `cc-rc-pr-update`'s PR summary/plan/ledger drafts
  - a `fix-tmp-perms` init container that sets the sticky bit on `/tmp` (mode `1777`;
    kubelet creates `emptyDir` volumes without it, and some applications refuse to write
    to a world-writable `/tmp` otherwise). It is the only container that runs as root.
...
Every workload this chart creates takes four extra-metadata knobs: `labels`/
`annotations` land on the workload object, and `podLabels`/`podAnnotations` land on its
pods. As with scheduling, there's no single global set — squid and the per-repo agents
are configured independently.

| | squid Deployment | per-repo agent StatefulSets |
| --- | --- | --- |
| workload object labels | `proxy.labels` | `labels`, `repos[].labels` |
| workload object annotations | `proxy.annotations` | `annotations`, `repos[].annotations` |
| pod labels | `proxy.podLabels` | `podLabels`, `repos[].podLabels` |
| pod annotations | `proxy.podAnnotations` | `podAnnotations`, `repos[].podAnnotations` |

The `repos[]` maps are merged **per key** over the top-level default — the same way
`repos[].resources` deep-merges over `resources` — so a repo can add or replace one
entry without repeating the shared set:
...
repos:
  - org: myorg
    repo: myrepo          # gets team=platform, cost-center=shared
  - org: myorg
    repo: gpu-repo
    labels:
      team: data          # replaces team; cost-center=shared still applies
    podAnnotations:
...
To reach one specific host inside those ranges (for example an internal registry on
10.x), list it in `proxy.defaultBlockedExceptions` rather than editing
...
whose DNS you trust. A CIDR exemption only covers requests for a raw IP in that range,
never a hostname that resolves into it — exempt the hostname itself.
...
| storage | object | `{"homeSize":"5Gi","storageClassName":"","tmpSize":"2Gi","workspaceSize":"5Gi"}` | Default persistent volume sizes for each per-repo agent. |
...
| storage.tmpSize | string | `"2Gi"` | sizeLimit of the emptyDir mounted at /tmp on the agent container (e.g. for cc-rc-pr-update's PR summary/plan/ledger drafts). |
